...
import { middleChemistryData, highChemistryData } from '@/data/chemistry'
...
export default function ChemistryPage() {
  ...
  const gradeDataMap: Record<string, any> = {
    middle: middleChemistryData,
    high: highChemistryData,
  }

  const currentData = gradeDataMap[selectedGrade]

  const gradeOptions = [
    { id: 'middle', name: '初中化学', icon: '🏫' },
    { id: 'high', name: '高中化学', icon: '🎓' },
  ]
            ...
            {!selectedSection ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-gray-500 max-w-md">
                  点击左侧章节名称<br />
                  然后选择小节查看知识点<br />
                  <span className="text-purple-600 font-semibold">💡 建议配合学习方法页面一起使用</span>
                </p>
                <Link
                  href="/methods"
                  className="mt-4 bg-purple-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-purple-600 transition"
                >
                  查看学习方法
                </Link>
              </div>
